...
.tooltip span, .tooltip div, .tooltip p {
    ...
    font-weight: 300;
            ...
            `;
            const gradients = `/* Animated gradient background for the toggle button */
...
.chat-toggle {
    ...
    /* Gradient background */
    background: linear-gradient(45deg, #686cff, #8083ff, #8c8efe, #0088ffc4, #686cff, #8083ff);
...
.chat-toggle:hover {
    transform: scale(1.1);
    box-shadow: 0 4px 15px rgba(104, 108, 255, 0.5);
}

/* Optional pulse effect in addition to the gradient */
@keyframes pulse-effect {
    0% {
        box-shadow: 0 0 0 0 rgba(104, 108, 255, 0.7);
    }
    70% {
        box-shadow: 0 0 0 10px rgba(104, 108, 255, 0);
    }
    100% {
        box-shadow: 0 0 0 0 rgba(104, 108, 255, 0);
    }
}

/* Add this class if you want to combine pulse with gradient */
.chat-toggle.with-pulse {
    animation: 
        gradient-animation 8s ease infinite,
        pulse-effect 2s infinite;
}

/* Spin animation for the logo inside the toggle button */
...
/* Regular bounce animation that happens every 2 seconds */
...
/* Apply the animation to the image inside the chat-toggle */
.chat-toggle img {
    width: 40px;
    height: 35px;
    object-fit: contain;
    /* Choose one of these animations: */
    animation: spin-animation 6s ease infinite;
    /* animation: bounce-spin 2s ease infinite; */
     /*animation: wiggle-animation 2s ease infinite;*/ 
}

/* Pause the animation when hovering over the button */
...
`;
            ...
            styleSheet.textContent = styles + additionalStyles + spinnerStyles + toggleStyles + connectionStatusStyles + maximizeStyles + scrollbar + sematics +sliderStyles + modalStyles + tooltip + gradients;
