    ...
    emit(eventName, data) {
        if (this.eventListeners.has(eventName)) {
            ...
            const eventData = {
                ...
                callReceiver: this.callReceiver,
            };
...
<!--                        <video id="localVideo" autoplay playsinline muted class="w-100 rounded" style="object-fit: contain; aspect-ratio: 16/9;"></video>-->
    ...
    async initiateVideoCall(targetuser,aux) {
        try {
           ...
           // console.log("Initiating call to:", targetuser);
            ...
            await iconnection.invoke("SendOffer", this.remoteEmail, UserObject.username, JSON.stringify(offer),aux);
    ...
    async handleOffer(callerEmail, offerStr, callType) {
